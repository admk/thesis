...
Arithmeitc expressions can be written in a linear form (\eg~$a + b$), or in
a tree structure (\eg~\binarymir{$+$}{$a$}{$b$}).  Similar to arithmetic
...
% as $[\varx \mapsto \varx]_{\varx \in \varset}$, the subscript $\varx \in
% \varset$ denotes that the \gls{mir} is constructed by collecting for each
% program variable $\varx$, the mapping shown in the bracket, \ie~the paring
% of the variable $\varx$ with the expression $\varx$.  The subscript $\varx
% \in \varset$ means that for each $\varx$, the \gls{mir} pairs it with $\varx
% \in \sexprset$, which is a semantic expression containing just the variable
% $\varx$ itself.  We will repeatedly make use of the notation of constructing
% a \gls{mir}, $[\varx \mapsto e(\varx)]_{p(\varx)}$ in the following sections,
% where $\varx$ is a variable and $p(\varx)$ is a predicate on $\varx$, the
% \gls{mir} is constructed by collecting for all $\varx$ the mapping from
% $\varx$ to the expression $e(\varx)$, when $p(\varx)$ is true.
...
The \gls{mir} of this example program allows a nested \gls{mir} to reference
and reuse an expression from the outer \gls{mir} as demonstrated in
Figure~\ref{po:fig:mir_fix_external_2}, where:
\begin{equation}
...
\begin{figure}[ht]
    ...
    \end{tikzpicture}
    \caption{%
        The \gls{mir} of Figure~\ref{po:lst:loop_share_example}, which
        reuses common subexpressions across nested \glspl{mir}.
