...
within an APEG is still exponentially complex.  For instance, consider an
...
to reduce the size of the search space.\footnote{The exact technique is
not explained in their original contribution.}  Additionally, as it was
...
The above heuristics utilize the static analysis of floating-point errors
introduced in Section~\ref{bg:sec:abstract_interpretation}.

Ioualalen~\etal~\cite{ioualalen} found their APEG approach can reduce round-off
errors in expressions by up to 50\%.

\subsubsection{Herbie}

Unlike the APEG approach which focuses on building an IR which can
...
only is it necessary to support sequential execution of straight-line code, but
also control-flow structures such as conditional branches and loops.
