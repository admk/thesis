...
\end{array}$
    \caption{%
        ...
        set-up; see Sec.~\ref{sec:results_discussion}.  We performed
        place-and-route for exact statistics.
    }\label{tab:results}
\end{table}

Table~\ref{tab:results} compares, for each benchmark in our evaluation set, the
performance metrics of the original program against those of the program with
the smallest latency discovered by our tool. We synthesized each program to a
circuit to obtain exact statistics, which are shown in Table~\ref{tab:results}.


\begin{figure}[ht]
    \subfloat[LUT count estimates] {%
        %\includegraphics[width=0.25\linewidth]{lut}

\begin{tikzpicture}
\begin{axis}[
  ylabel={\small Estimated},
  xlabel={\small Actual},
  height=46mm, width=46mm, ylabel near ticks,
...
\pgfplotstableread[col sep = comma]{csv/lut.csv}\mydata

\addplot[draw=none, mark=*, draw opacity=0, fill opacity=0.5]
table [y=Estimate, x=Actual] \mydata;

\addplot[mark=none, draw=none] table [x=Actual, y={create col/linear
regression={y=Estimate}}] \mydata;
...
\label{fig:lut}
    ...
    \subfloat[Latency estimates] {%
        %\includegraphics[width=0.25\linewidth]{lat}
\begin{tikzpicture}
\begin{loglogaxis}[
  ylabel={\small Estimated},
  xlabel={\small Actual},
...
\pgfplotstableread[col sep = comma]{csv/lat.csv}\mydata
...
\addplot [no markers, domain=1000:100000000000] {\intercept+\slope*x};
...
\label{fig:lat}
    ...
    \caption{%
        Comparisons of our estimated resource and latency statistics against
        Vivado HLS\@.}
\end{figure}
...
\centering
\tikzset{%
  every axis/.style={height=60mm, width=60mm, ylabel near ticks, try min ticks=5},
...
\pgfplotstableread[col sep=comma, row sep=newline]{csv/seidel.csv}\seidel
\begin{tikzpicture}
\begin{axis}[
...
\caption{%
    ...
    Fig.~\ref{fig:seidel_prog}. Each graph shows a 2D projection of the 3D
    Pareto frontier. In each graph, the original program is marked $\times$,
    and the lowest-latency variant obtained by arithmetic transformations alone
    is marked \protect\redcircle. }
\label{fig:seidel}
\end{figure*}
