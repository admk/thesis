...
Our first contribution is that instead of optimizing the \gls{mir}
immediately, we start by partitioning the \gls{mir} into multiple smaller
sub-\glspl{mir}.  The partition boundaries are determined by $\updateop$
operators.  For instance, we consider the partially unrolled Fibonacci example
in Figure~\ref{lo:fig:fib}.  The \gls{mir} of the loop body is shown in
Figure~\ref{lo:fig:fib_mir2}.  The partition boundaries are indicated by the
region surrounded by the red dotted curve \tikz{\draw[red, dashed, rounded
corners=1ex] (0,0) rectangle (20pt,2ex);}.
\begin{figure}[ht]
    \centering
    \newsavebox{\fiblstb}
    \begin{lrbox}{\fiblstb}
...
for (int i=3; i<1023; i+=2)
{
    ...
    \subfloat[The \gls{mir} of the partially unrolled loop body.]{%
        \begin{tikzpicture}[mir]
            ...
            \node[mirnode] (-2) [below right=6mm of 2acc2] {$-$};
            ...
            \draw[<-] (-2) -- (2);
            \draw[<-] (2upd) -- (Ain);
            \draw[<-] (2upd) to[out=-70, in=180] (-1);
            \draw[<-] (2acc1) -- (Ain);
            \draw[<-] (2acc2) -- (Ain);
            \draw[<-] (-3) -- (-3i);
            \draw[<-] (-2) -- (-2i);

            \draw[<-] (acc2) to[bend left] (-2);
            ...
            \draw[red, dashed, rounded corners]
                (-3i.south) -- (Ain.south west) -- (Ain.north west) --
                (2upd.north west) -- (2upd.north east) --
                (2acc2.north east) -- (2.north east) --
                (2.south east) -- cycle;
            \draw[red, dashed, rounded corners]
                (-1i.south) -- (acc1.south west) -- (acc1.north west) --
                (upd.north west) -- (upd.north east) --
                (acc2.north east) -- (1.south east) -- cycle;
            \draw[red, dashed, rounded corners]
    ...
    \caption{An example to illustrate how \glspl{mir} are partitioned.}
\end{figure}

Because transformation rules can only be applied to each sub-\gls{mir} but not
