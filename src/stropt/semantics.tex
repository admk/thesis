...
In this paper, the function $\error: \exprset\to\errorset$ is used to represent
...
by one step of (\ref{eq:equivalence_inference}), \ie~$\eqstep(\epsilon) =
\{e^\prime\in\exprset \mid e \eqrel e^\prime \wedge e\in\epsilon\}$, where
$\epsilon$ is a set of equivalent expressions. From this, we may note that
the set of equivalent expressions generated by taking the union of any
